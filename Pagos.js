...
/**
* (PASO 1)
* 🛑 ACTUALIZADO: Acepta un array de 'hermanos' y los registra por separado.
* El check de Foto Carnet se movió a 'registrarDatos'.
*/
function paso1_registrarRegistro(datos, hermanos = []) {
try {
// 1. Definir el estado de pago
let estadoPagoStr = "Pendiente";
if (datos.metodoPago === 'Pago Efectivo') {
estadoPagoStr = "Pendiente (Efectivo)";
} else if (datos.metodoPago === 'Pago en Cuotas') {
estadoPagoStr = `Pendiente (${datos.cantidadCuotas} Cuotas)`;
}
datos.estadoPago = estadoPagoStr;

// 2. Registrar al inscripto PRINCIPAL
const respuestaRegistro = registrarDatos(datos, false); // false = no es hermano

// Si el principal falla (p.ej. cupo, cerrado, foto), detener todo
if (respuestaRegistro.status !== 'OK_REGISTRO') {
return respuestaRegistro;
}

// 3. Registrar a los HERMANOS (si existen)
if (hermanos && hermanos.length > 0) {
Logger.log(`Registrando ${hermanos.length} hermanos...`);

for (const hermano of hermanos) {
try {
const datosHermano = {
// Datos únicos del hermano
nombre: hermano.nombre,
apellido: hermano.apellido,
dni: hermano.dni,
fechaNacimiento: hermano.fechaNacimiento,
colegioJardin: hermano.colegioJardin,

// Datos copiados del principal
tipoInscripto: datos.tipoInscripto,
email: datos.email,
adultoResponsable1: datos.adultoResponsable1,
dniResponsable1: datos.dniResponsable1,
telefonoResponsable1: datos.telefonoResponsable1,
adultoResponsable2: datos.adultoResponsable2,
telefonoResponsable2: datos.telefonoResponsable2,
personasAutorizadas: datos.personasAutorizadas,
jornada: datos.jornada,
metodoPago: datos.metodoPago,
cantidadCuotas: datos.cantidadCuotas,
estadoPago: estadoPagoStr, // Mismo estado de pago

// Datos de salud y archivos (se completarán después)
obraSocial: '',
practicaDeporte: '',
especifiqueDeporte: '',
tieneEnfermedad: '',
...
urlFotoCarnet: '' // Se pedirá al momento de pagar
};

// Registrar como hermano (true), saltando el check de foto
const respHermano = registrarDatos(datosHermano, true);
if (respHermano.status !== 'OK_REGISTRO') {
Logger.log(`Error al registrar hermano ${hermano.dni}: ${respHermano.message}`);
// No detenemos el proceso, solo logueamos el error.
} else {
Logger.log(`Hermano ${hermano.dni} registrado con Turno #${respHermano.numeroDeTurno}`);
}

} catch (eHermano) {
Logger.log(`Error crítico al procesar hermano ${hermano.dni}: ${eHermano.message}`);
}
}
}

// Devolver la respuesta del inscripto PRINCIPAL
return respuestaRegistro;
...
// 🛑 ACTUALIZADO: Constantes y campos de nombre
// =========================================================
...
// 🛑 ACTUALIZADO: Constantes de columnas
// =========================================================
...
// Llamadas a los helpers (🛑 Constantes actualizadas)
