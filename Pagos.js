...
function paso2_crearPagoYEmail(datos, numeroDeTurno) {
  try {
    ...
    if (pagosHabilitados === false) {
      Logger.log(`Pagos deshabilitados (Config B23). Registrando sin link. Turno #${numeroDeTurno}`);
      enviarEmailConfirmacion(datos, numeroDeTurno, null, 'registro_sin_pago');
      return { status: 'OK_REGISTRO_SIN_PAGO', message: `¡Inscripción registrada (Turno #${numeroDeTurno})! Los pagos online están momentáneamente desactivados. Por favor, acérquese a la administración.` };
    }
    ...
    if (datos.metodoPago === 'Pago Online') {
      ...
      try {
        ...
        return { status: 'OK_REGISTRO_SIN_LINK', message: `¡Tu registro se guardó (Turno #${numeroDeTurno})! Pero falló la creación del link de pago. Por favor, contacte a la administración para abonar.` };
      }
    ...
    if (datos.metodoPago === 'Pago en Cuotas') {
      ...
      try {
        ...
        return { status: 'OK_REGISTRO_SIN_LINK', message: `¡Tu registro se guardó (Turno #${numeroDeTurno})! Pero falló la creación de los links de pago. Por favor, contacte a la administración.` };
      }
      ...
      if (!primerLink || !primerLink.startsWith('http')) {
         return { status: 'OK_REGISTRO_SIN_LINK', message: `¡Registro guardado (Turno #${numeroDeTurno})! ${primerLink}` };
      }
